...
def qaCzsc():
    ...
    te = QA.QA_fetch_stock_day_adv('000010',start_date,end_eDate).to_qfq()
    ...
    for i in range(start_index,end_index):
        ...
        zs2 = zs2List(zslist,bars)
        ka_to_image2(ka,'d:\\abc.jpg',zs= zs2)
        exit()
    ...
    3、次级别背驰的多种验证方法
    '''
    te = QA.QA_fetch_stock_day_adv('000001','2022-01-01',end_eDate).to_qfq()
    bars = te.data.round(2).reset_index()
    bars.rename(columns={'code': 'symbol', 'date': 'dt'}, inplace=True)
    ka = KlineAnalyze(bars,)
    zslist = find_zs(ka.bi_list)

    if ka.bi_list[-1]['fx_mark']=='d' and str(ka.bi_list[-1]['end_dt'])[:10]==end_date: #大趋势刚出现底分型，
    ...
    f.close()
    '''
    min60 = QA.QA_fetch_stock_min_adv('000001','2022-09-15','2022-11-04',frequence='30min').to_qfq()
